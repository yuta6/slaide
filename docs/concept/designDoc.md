...
[build-png.mjs]      [build-pdf.mjs]            [build-html.mjs]
    ↓                    ↓                          ↓
[Playwright]         [Playwright]               [HTML結合 + Presenter注入]
    ↓                    ↓                          ↓
...
interface Props {
  slides: { html: string; transition: string; notes: string; order: number }[];
  deckTitle?: string;
}
...
```
1. 引数パース（--deck <n> | --all）
2. Astro ビルド済み dist/ からデッキの HTML ファイルを取得
3. ファイル名でソート
4. 既定のスライドサイズを決定
5. Playwright Chromium 起動
6. 各 HTML を開いて page.screenshot() で PNG 生成
...
async function buildPng(deckName: string) {
  ...
  for (const file of files) {
    ...
    await page.screenshot({
      path: pngPath,
      fullPage: false,  // 固定枠サイズでキャプチャ（スクロールしない）
      type: 'png',
    });
...
Claude Code がスライド生成
  ...
  → dist/<n>/png/01.png, 02.png, ... が生成される
...
2. Astro ビルド済み dist/ からデッキのHTMLファイルを取得
3. ファイル名でソート（01.html, 02.html, ...）
4. 既定のページサイズを決定
5. Playwright Chromium 起動
6. 各HTMLを開いて page.pdf() で個別PDF生成
...
2. Astro ビルド済み dist/ からデッキのHTMLファイルを取得
3. ファイル名でソート
4. 各HTMLから <body> の中身を抽出
5. src/presenter/runtime.ts をバンドル（esbuild）
6. src/presenter/transitions.css を読み込み
7. 全スライドを <section data-slide> で包んで結合
8. Presenter ランタイム + CSS を注入
9. 画像・フォントを Base64 インライン化
...
<body>
  <div class="presenter-viewport">
    <div class="slide-scaler">
      <!-- スライド1 -->
      <section data-slide data-slide-order="1" data-slide-transition="fade" data-slide-notes="...">
        {slide_01_html}
      </section>

      <!-- スライド2 -->
      <section data-slide data-slide-order="2" data-slide-transition="slide">
        {slide_02_html}
      </section>
...
- Astro ビルドの出力HTML からスライド本体のHTMLだけを抽出する（`<head>` の `<style>` も必要）
...
`build.format: 'file'` にすることで、`src/pages/pitch-deck/01.astro` → `dist/pitch-deck/01.html` になる。ビルドスクリプトがファイルを列挙しやすい。
...
1. **Astro scoped CSS の抽出**: build-html.mjs が各スライドの scoped CSS を正しく抽出して結合HTMLに含められるか
