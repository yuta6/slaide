...
- **ビルドスクリプト**: Astro build → Playwright → PDF / 単一 HTML 結合
...
│   └── build-html.mjs          ← 全スライド → 単一 HTML 結合
│
├── assets/                     ← 画像・SVG・素材置き場
│   └── fonts/                  ← ユーザーが用意するフォント
│
...
src/pages/AGENTS.md             ← スライドの書き方・frontmatter・命名規則
```
...
- スライド固有の値は frontmatter に書く
- 将来、PDF author や HTML title などビルド専用のメタデータが必要になった場合だけ、別ファイルを追加検討する

### pages/ のルール

- `src/pages/` 直下のディレクトリが1つのデッキ
- 各デッキ内のファイル名は `01.astro`, `02.astro`, ... の番号管理（ゼロパディング2桁）
- タイトルスライドは `01.astro`、クロージングは最後の番号
- ビルドスクリプトはファイル名の数値ソートでページ順を確定する

### frontmatter

各スライドは frontmatter にメタデータを記述する。全フィールドはオプショナル。

```astro
---
order: 1
title: "市場分析レポート"
transition: "fade"            # HTML出力時: fade | slide | none
theme: "dark"                 # テーマ上書き
showHeader: true
showFooter: true
notes: "スピーカーノート"
...
Astro build（静的 HTML 生成）
  → 指定デッキのページを Playwright で開く（ファイル名ソート順）
  → 各ページを PDF 化（page.pdf()）
...
4. サンプルデッキ1つ（sample/ に 01.astro 〜 03.astro）
5. build-pdf.mjs（Playwright → PDF、--deck 対応）
6. build-html.mjs（単一 HTML 結合 + ナビゲーション、--deck 対応）
